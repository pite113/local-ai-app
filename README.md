...
- 💬 **AI 对话**：网页聊天界面，像本地版 ChatGPT；**聊天记录自动保存**（刷新不丢）；知识库可开关
- 📚 **知识库问答（RAG）**：上传文档 → 句子级切片 → 语义检索 → 自动引用回答；支持 **txt/md/csv/json/log + Word(.docx) + PDF**
- 🧰 **工具中心**：批量文本处理（**并发+去重**省 token）、批量作图、表格**多列**批量处理
...
| 工具 | 用法 | 说明 |
|---|---|---|
| ① 批量文本 | 每行一条文本 → 选"改写/翻译/生成" → 运行 → 复制/下载 | **并发处理**（`TOOL_CONCURRENCY`，默认3）；**相同内容自动去重**，只调用一次模型 |
| ② 批量作图 | 每行一个提示词 → 选尺寸 → 生成 → 下载 | 默认"占位图"模式测流程；接生图 API 后自动切换真实生图 |
| ③ 表格处理 | 上传 Excel/CSV → 解析 → **Ctrl 多选列** → 处理 → 下载结果 | 每列生成"处理结果_列名"新列；支持 xlsx/csv、UTF-8/GBK |
...
- **一键打包**：双击 `一键打包.bat` → 自动生成"交付包"zip（已排除 `.env` 密钥/环境/数据），U盘拷给客户即可
- **部署手册**：见 `DEPLOY.md`（傻瓜化步骤：装 Python → 解压 → 配 .env → 启动 → 自启 → 内网访问）
